...
void Solve() {
    ...
    // Input
    vector<pair<int, int>> buttonA, buttonB, prize;
    string line;
    ...
    //     frange(j,100) {
    //         frange(k,100) {
    //             // (c1 - ax1)/y1 = b which is an int
    //             // and (c1 - by1)/x1 = a
    //             // c2 = ax2 + by2
    //             int x = j*(a[i].f) + k*(b[i].f);
    //             int y = j*(a[i].s) + k*(b[i].s);
    //             if (x == c[i].f && y == c[i].s) {
    //                 ans += j*3 + k*1;
    //                 goto next;
    //             }
    ...
    frange(i, n) {
        i128 x = 0, y = 0, g = 0;
        x = ((c[i].f*b[i].s) - (c[i].s*b[i].f))/((a[i].f*b[i].s) - (a[i].s*b[i].f));
        y = ((c[i].f*a[i].s) - (c[i].s*a[i].f))/((b[i].f*a[i].s) - (b[i].s*a[i].f));
        if (c[i].f == x*a[i].f + y*b[i].f && c[i].s == x*a[i].s + y*b[i].s)
            ans += 3*x + y;
    }
